...
## How to use intro/outro auto-skip and how does recap skipping work?

Nova Video Player supports automatic skipping of video intro, outro, credits, and preview segments based on online database metadata (retrieved via TMDb and TV show IMDb identifiers).

**Where to find the setting:**
* **Phone / Tablet UI**: During video playback, open the playback menu → select **Play mode** → toggle **"Skip intro/outro"**;
* **TV UI (Leanback)**: Press D-Pad Up or Menu during playback → navigate to the **Play mode** tile → toggle **"Skip intro/outro"**.

**Recap Skipping Logic in Binge Mode:**
* Recaps (previous episode summaries) do not require a separate toggle.
* When **"Skip intro/outro"** is enabled, recaps are automatically skipped **only during binge-watching** when advancing automatically from one episode to the next (in **Binge watching** play mode).
* The first episode manually started in a session retains its recap so you don't lose context.

## What are home screen categories and smart category decluttering?

Nova Video Player allows customizing home screen categories in Nova Settings:

**General UI Options (Phone/Tablet & TV):**
Available under **Settings → User interface**:
* **Hide watched videos**: Hides completed videos from home and category views;
* **Smart home rows decluttering**: Applies to both Phone/Tablet and TV interfaces, transforming standard categories into decluttered smart rows:
  * **New to watch** (replaces *Recently added*): Displays only completely unwatched content and groups items by show/movie to prevent multiple episode additions from crowding the row;
  * **Keep watching** (replaces *Recently played*): Displays played content, grouping episodes by TV show and keeping only the single most recently played episode per show to prevent series duplication.

**TV UI (Leanback) Specific Options:**
Available on TV devices under **Settings → Leanback user interface**:
* **Separate Anime from Movies/TV Shows**: Isolates animated content into a dedicated **Anime** category row on the TV home screen;
* **Row Customization**: Toggle visibility of individual rows (*Recently added*, *Recently played*, *Movies*, *TV Shows*, *Anime*) and configure custom sort orders for each row on the TV main screen.
